...
var NRS = (function(NRS, $, undefined) {
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@qaq/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@qaq/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(BURST\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@qaq";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			if (uri) {
				var match = uri.match(/acct:(.*)@qaq/i);
				if (!match) {
