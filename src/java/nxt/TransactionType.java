...
public abstract class TransactionType {
            ...
            case TYPE_BURST_MINING:
            	switch (subtype) {
            		case SUBTYPE_BURST_MINING_REWARD_RECIPIENT_ASSIGNMENT:
            			return QaqMining.REWARD_RECIPIENT_ASSIGNMENT;
            		default:
            			return null;
            	}
    ...
    */
    ...
    public static abstract class QaqMining extends TransactionType {
    	...
    	public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new QaqMining() {
    		...
    		@Override
            Attachment.QaqMiningRewardRecipientAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException {
                return new Attachment.QaqMiningRewardRecipientAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.QaqMiningRewardRecipientAssignment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException {
                return new Attachment.QaqMiningRewardRecipientAssignment(attachmentData);
            }
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates) {
                ...
                return isDuplicate(QaqMining.REWARD_RECIPIENT_ASSIGNMENT, Convert.toUnsignedLong(transaction.getSenderId()), duplicates);
            }
		...
		public final static TransactionType ESCROW_CREATION = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(transaction.getFeeNQT() < Constants.ONE_NXT) {
					throw new NxtException.NotValidException("Escrow transaction must have a fee at least 1 qaq");
				}
		...
		public final static TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(transaction.getAmountNQT() < Constants.ONE_NXT || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
					throw new NxtException.NotValidException("Subscriptions must be at least one qaq");
				}
				if(transaction.getSenderId() == transaction.getRecipientId()) {
